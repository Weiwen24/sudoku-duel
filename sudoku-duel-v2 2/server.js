...
const PUZZLES = {
  Easy: [
    ...
    {
      grid:     '200000600300200005060030020000800300070000090004005000030060040700001006008000001',
      solution: '284917635391286475567034928156849372873562194924375861435621749719453286648790513',
      diff: 'Easy'
    }
  ],
  Medium: [
    {
      grid:     '000000000010020030005600700004030200070800010003050600500060100080040050000000000',
      solution: '637418529918527436245639718764931285572846913893752641356284197189473652421965873',
      diff: 'Medium'
    },
    {
      grid:     '020000000000600003074080000000003002080040010600500000000010780500009000000000040',
      solution: '126437958895621473374985126457813962983246517612579384249168735531792648768354291',
      diff: 'Medium'
    },
    {
      grid:     '600120390008095061902000005007040802050090010408050700900000207860450109037081004',
      solution: '645127398718395461932864175197643852253978614486251739914536287861742953375819024',
      diff: 'Medium'
    }
  ],
  Hard: [
    {
      grid:     '800000000003600000070090200060005300400080700100004056009500001800200090007600000',
      solution: '812753649943682175675491283168945372439278751752164836926537418581324967347816523',
      diff: 'Hard'
    },
    {
      grid:     '200080300060070084030500209000105408000000000402706000301007040720040060004010003',
      solution: '245189376169273584837564219976125438518394762423786951391657842752948163684312795',
      diff: 'Hard'
    },
    {
...
// ─── Room State ──────────────────────────────────────────────────────────────
// rooms: { [code]: { code, puzzle, status, players: [], startTime } }
const rooms = {};
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({ name, difficulty }) => {
    ...
    const puzzle = randomPuzzle(diff);
    rooms[code] = {
      code,
      puzzle,
      difficulty: diff,
  ...
  socket.on('start_game', () => {
    ...
    room.puzzle = randomPuzzle(room.difficulty); // fresh puzzle on each start
    io.to(code).emit('game_started', { room: sanitizeRoom(room) });
    console.log(`Game started in room ${code}`);
  });

  // PROGRESS UPDATE (client sends progress %)
...
server.listen(PORT, () => {
  console.log(`Sudoku Duel server running on port ${PORT}`);
});
